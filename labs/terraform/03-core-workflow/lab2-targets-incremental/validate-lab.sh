...
# 5. Al menos cuatro recursos local_file definidos
...
if [ "$RESOURCE_COUNT" -ge 4 ]; then
    ...
    echo -e "${RED}FAIL${NC} - Se esperan al menos 4 recursos, encontrados: ${RESOURCE_COUNT}"
    ((FAILED++))
fi

# 6. Existe al menos una referencia de dependencia entre recursos
if grep -q 'local_file\.' "$PROJECT_DIR/main.tf" 2>/dev/null; then
    echo -e "${GREEN}PASS${NC} - Dependencias entre recursos encontradas en main.tf"
    ((PASSED++))
else
    echo -e "${RED}FAIL${NC} - No se encontraron referencias de dependencia en main.tf"
    ((FAILED++))
fi
...
if [ "$FAILED" -eq 0 ]; then
    ...
    echo "$(date +%Y-%m-%d\ %H:%M:%S)" > "/root/.badge-terraform-m3-lab2-earned"
    exit 0
else
    echo ""
